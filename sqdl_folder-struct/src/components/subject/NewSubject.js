...
const NewSubject = () => {
  ...
  async function submissionHandler(){
    ...
    axios.post(GLOBAL_URL + 'subject/create', { name: subject.name, description: subject.description, createdBy: subject.createdBy}, res)
    .then((response=>{
      setSubject({...subject, disabled: true, msg:'Created Subject Successfully'})
      //add too cookie
      const user = check()
      ...
      <div className='align-center p-10 flex flex-col items-center h-screen '>
        ...
        <div className='border-blue-400 border-4 rounded-lg p-5 py-10 items-center justify-center flex'>
          <Card color="transparent" shadow={false}>
            ...
            <Typography className= 'text-center text-red-500'>{subject.msg}</Typography>
            <form className='mt-8 mb-2 w-80 max-w-screen-lg sm:w-96'>
              <div className='mb-4 flex flex-col gap-6'>
                <Input label='Name' onChange={(e)=>{setSubject({...subject, name:e.target.value})}} value={subject.name}></Input>
                <Textarea label='Description' onChange={(e) => { setSubject({ ...subject, description: e.target.value }) }} value={subject.description}></Textarea>
                <Button disabled={(subject.name == '') || (subject.description == '')||subject.disabled}onClick={() => { submissionHandler() }}>Create</Button>
              </div>
            </form>
          </Card>
        </div>
