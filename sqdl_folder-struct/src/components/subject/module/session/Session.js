...
const Session = () => {
  ...
  function handleSub() {
    ...
    axios.post(GLOBAL_URL + 'session/update', { title: title, description: desc, _id: params.sessionid, topic: topic, description: desc, conductedBy: conductedBy, enrollmentLimit: enrollmentLimit, activity_order: session.copy })
      .then((response) => {
        console.log(response)
        setSession({ ...session, title: response.data.data.title, description: response.data.data.description, topic: response.data.data.topic, enrollmentLimit: response.data.data.enrollmentLimit, conductedBy: response.data.data.conductedBy, activity_order: response.data.data.activity_order, copy: response.data.data.activity_order  })
        closeDrawer()
      })
      .catch((error) => {
        console.log(error)
  ...
  async function getAllData() {
    ...
    axios.post(GLOBAL_URL + 'user/getID', { _id: check()._id }, res)
      ...
      .then(() => { //requesting sessiondata
        return axios.post(GLOBAL_URL + 'session/get', { _id: params.sessionid }, res)
      })
      .then((response) => {       //updating duplicate state hook
      ...
      <div className='align-center p-10 flex flex-col items-center h-screen '>
        ...
        <Card className="mt-6 w-2/5 p-5">
          <div className='text-center'>
            ...
            <hr></hr><br />
            <Typography variant='paragraph' className='text-left'>Description: {session.description}</Typography>
            <Typography variant='paragraph' className='text-left'>Conducted by: {session.conductedBy}</Typography>
            <hr/>
            <Typography className = 'font-semibold text-left'>Activities</Typography> 
            {
        ...
        <Drawer open={open} onClose={closeDrawer} className="p-4" size={400}>
          ...
          <form className="flex flex-col gap-6 p-4">
            ...
            <Input id='conductedBy' label="Conducted By" defaultValue={session.conductedBy} />
            <Input id='enrollmentLimit' label="Enrollment Limit" defaultValue={session.enrollmentLimit} />
            <ActivitySelect/>
